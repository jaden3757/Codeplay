...
def maprun():
    ...
    buttonmode = 0
    setscr(0)
    sheetname = 'b_manageroom' # 엑셀파일에 자신이 원하는 방의 이름을 시트로 추가 (건드려야할 것)
    floor_button.item = [sheetname, 1] # 엑셀파일의 'sp3'시트의 1번째 가로줄을 할당

    # | 여기부터 자유롭게 추가 또는 변경 |
    holy = itemobject("light2.png", "빛", 100, 100, 200, 200) # 예시
    holy.item = [sheetname, 1] # 엑셀파일의 'sp3'시트의 2번째 가로줄을 할당
    ...
    com_button = button("살펴본다", 300, 40, 650, 200) # 하위 버튼 디자인
    com_button.color = (0,0,0)
    com_button.textsize = 20
    com_button.font = 'pixel.ttf'

    b_hall_button = button("B 홀", 300, 40, 650, 200) # 하위 버튼 디자인
    b_hall_button.color = (0,0,0)
    b_hall_button.textsize = 20
    b_hall_button.font = 'pixel.ttf'
    
    b_long_button = button("B Long", 300, 40, 650, 250) # 하위 버튼 디자인
    b_long_button.color = (0,0,0)
    ...
    while run:
        ...
        # | UI |
        prtext4("B 관리실 | B-4", 'pixel.ttf', 20, 30, 30) # 여기는 바꿔도 됨
        drawui()
        textls()
        textprinting()
