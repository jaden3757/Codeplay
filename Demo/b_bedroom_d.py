...
def maprun():
    ...
    floor_button.item = [sheetname, 10] # 엑셀파일의 'sp2'시트의 1번째 가로줄을 할당
    ...
    # bed.item = [sheetname, 18]
    # drawer.item = [sheetname, 19]
    # desk.item = [sheetname, 20]
    # box.item = [sheetname, 21]
    drawer = itemobject('images/none.png', '서랍', 200, 200, 150, 150)
    drawer.item = [sheetname, 6]
    ...
    bgimg = pygame.image.load('images/bedroom.png')
    ...
    while run:
        ...
        # UI
        prtext4("B 침실 - D 방 | B-1", 'pixel.ttf', 20, 30, 30) # 여기는 바꿔도 됨
        drawui()
        textls()
        textprinting()
