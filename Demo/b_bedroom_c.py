...
def maprun():
    ...
    floor_button.item = [sheetname, 12] # 엑셀파일의 'sp2'시트의 1번째 가로줄을 할당
    ...
    # bed.item = [sheetname, 13]
    # drawer.item = [sheetname, 14]
    # desk.item = [sheetname, 15]
    # box.item = [sheetname, 16]
    drawer = itemobject('images/none.png', '서랍', 200, 200, 150, 150)
    drawer.item = [sheetname, 5]
    ...
    while run:
        ...
        # UI
        prtext4("B 침실 - C 방 | B-1", 'pixel.ttf', 20, 30, 30) # 여기는 바꿔도 됨
        drawui()
        textls()
        textprinting()
